...
class WP_REST_Global_Styles_Revisions_Controller_Test extends WP_Test_REST_Controller_Testcase {
	...
	public function set_up() {
		...
		if ( is_multisite() ) {
			...
			grant_super_admin( self::$second_admin_id );
			WP_Theme::network_enable_theme(
				array(
					'tt1-blocks',
					'block-theme-child-with-block-style-variations',
				)
			);
		}
	...
	public function tear_down() {
		if ( is_multisite() ) {
			...
			revoke_super_admin( self::$second_admin_id );
		}
	...
	#[\PHPUnit\Framework\Attributes\Ticket( '59810' )]
	#[\PHPUnit\Framework\Attributes\Ticket( '56481' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_METHOD, 'WP_REST_Revisions_Controller', 'get_revision' )]
	...
	#[\PHPUnit\Framework\Attributes\DataProvider( 'data_readable_http_methods' )]
	#[\PHPUnit\Framework\Attributes\Ticket( '58524' )]
	#[\PHPUnit\Framework\Attributes\Ticket( '60131' )]
	#[\PHPUnit\Framework\Attributes\Ticket( '56481' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_METHOD, 'WP_REST_Global_Styles_Controller', 'get_item_permissions_check' )]
	 ...
	 *
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '64292' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_METHOD, 'WP_REST_Global_Styles_Revisions_Controller', 'prepare_item_for_response' )]
	public function test_get_item_preserves_block_style_variations() {
		...
		// A revision stores the content that is being replaced. Trigger another
		// update so the revision under test contains the variation config above.
		wp_update_post(
			array(
				'ID'         => $global_styles_id,
				'post_title' => 'Custom Styles Updated',
			),
			true
		);
		...
		foreach ( $revisions as $candidate ) {
			$candidate_content = json_decode( $candidate->post_content, true );
			if ( isset( $candidate_content['styles']['blocks']['core/group']['variations'] ) ) {
				$revision = $candidate;
				break;
			}
		}
		$this->assertInstanceOf( WP_Post::class, $revision, 'A revision containing block style variations should exist.' );
	 ...
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '64292' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_METHOD, 'WP_REST_Global_Styles_Revisions_Controller', 'get_items' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_METHOD, 'WP_REST_Global_Styles_Revisions_Controller', 'prepare_item_for_response' )]
