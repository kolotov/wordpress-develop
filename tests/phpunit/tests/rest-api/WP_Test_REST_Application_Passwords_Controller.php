...
class WP_Test_REST_Application_Passwords_Controller extends WP_Test_REST_Controller_Testcase {
	...
	public function set_up() {
		...
		$this->server_state = $this->capture_state( $_SERVER, array( 'PHP_AUTH_USER', 'PHP_AUTH_PW', 'HTTPS' ) );
		$this->global_state = $this->capture_state( $GLOBALS, array( 'wp_rest_application_password_status', 'wp_rest_application_password_uuid', 'current_user' ) );

		add_filter( 'wp_is_application_passwords_available', '__return_true' );
	}

	public function tear_down() {
		$this->restore_state( $_SERVER, $this->server_state );
		$this->restore_global_state( $this->global_state );
		parent::tear_down();
	}

	private function capture_state( $source, $keys ) {
		$state = array();
		foreach ( $keys as $key ) {
			$state[ $key ] = array(
	...
	private function restore_state( &$target, $state ) {
	...
	private function restore_global_state( $state ) {
		foreach ( $state as $key => $entry ) {
			if ( $entry['exists'] ) {
	 ...
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '53224' )]
	...
	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '51583' )]
	...
	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '53692' )]
	...
	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '52275' )]
	 ...
	 *
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '53658' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_FUNCTION, 'wp_is_application_passwords_supported' )]
	 ...
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '53658' )]
	#[\PHPUnit\Framework\Attributes\DataProvider( 'data_wp_is_application_passwords_available' )]
	#[WP_PHPUnit_Covers( WP_PHPUnit_Covers::TARGET_FUNCTION, 'wp_is_application_passwords_available' )]
	 ...
	 * @return array
	 */
	public static function data_wp_is_application_passwords_available() {
		return array(
