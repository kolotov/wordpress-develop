...
/**
 */
#[\PHPUnit\Framework\Attributes\Group( 'https-detection' )]
class Tests_HTTPS_Detection extends WP_UnitTestCase {
	...
	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '47577' )]
	public function test_wp_is_using_https() {
		...
		$force_http = $this->filter_set_url_scheme( 'http' );
		add_filter( 'site_url', $force_http );
		$this->assertFalse( wp_is_using_https() );
		remove_filter( 'site_url', $force_http );
	}
	...
	public function test_wp_is_https_supported() {
		...
		$no_errors = static function () {
			return new WP_Error(); // No errors means HTTPS is supported.
		};
		add_filter( 'pre_wp_get_https_detection_errors', $no_errors );

		// No errors, so HTTPS is supported.
		$this->assertTrue( wp_is_https_supported() );
		remove_filter( 'pre_wp_get_https_detection_errors', $no_errors );
		...
		$with_errors = static function () use ( $support_errors ) {
			return $support_errors;
		};
		add_filter( 'pre_wp_get_https_detection_errors', $with_errors );

		// Test that HTTPS is not supported due to the simulated errors.
		$this->assertFalse( wp_is_https_supported() );

		remove_filter( 'pre_wp_get_https_detection_errors', $with_errors );
	}

	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '47577' )]
	#[\PHPUnit\Framework\Attributes\Ticket( '52542' )]
