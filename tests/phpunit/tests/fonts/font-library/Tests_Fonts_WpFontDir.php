...
#[\PHPUnit\Framework\Attributes\CoversFunction( 'wp_get_font_dir' )]
class Tests_Fonts_WpFontDir extends WP_UnitTestCase {
	...
	public function test_fonts_dir_for_multisite() {
		...
		$expected        = array(
			'path'    => untrailingslashit( $site_upload_dir['basedir'] ) . '/fonts',
			'url'     => untrailingslashit( $site_upload_dir['baseurl'] ) . '/fonts',
			'subdir'  => '',
			'basedir' => untrailingslashit( $site_upload_dir['basedir'] ) . '/fonts',
			'baseurl' => untrailingslashit( $site_upload_dir['baseurl'] ) . '/fonts',
			'error'   => false,
		);
	 ...
	 *
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '60652' )]
