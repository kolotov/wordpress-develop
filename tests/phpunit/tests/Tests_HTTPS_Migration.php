...
/**
 */
#[\PHPUnit\Framework\Attributes\Group( 'https-migration' )]
class Tests_HTTPS_Migration extends WP_UnitTestCase {
	private $original_options = array();
	private $missing_option;

	public function set_up() {
		parent::set_up();
		$this->missing_option = new stdClass();

		foreach ( array( 'home', 'siteurl', 'https_migration_required', 'fresh_site' ) as $option ) {
			$this->original_options[ $option ] = get_option( $option, $this->missing_option );
		}
	}

	public function tear_down() {
		foreach ( $this->original_options as $option => $value ) {
			if ( $this->missing_option === $value ) {
	...
	/**
	 */
	#[\PHPUnit\Framework\Attributes\Ticket( '51437' )]
